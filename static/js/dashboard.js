/* ======================================================
   ...
   ADDED: updatePaymentStatus function for orders
====================================================== */
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ========== ✅ ADDED: UPDATE PAYMENT STATUS FUNCTION ==========
    window.updatePaymentStatus = function(select, orderId) {
        ...
        select.disabled = true;
        window.showToastNotification("Updating payment status...", "info");
        ...
        .then(data => {
            ...
            if (data.status === "ok") {
                window.showToastNotification("✅ Payment updated successfully!", "success");
                if (data.whatsapp_link) window.open(data.whatsapp_link, "_blank");
                setTimeout(() => location.reload(), 1000);
            } else {
                window.showToastNotification("❌ Update failed: " + (data.error || "Unknown error"), "error");
            }
            select.disabled = false;
            select.style.opacity = '1';
        })
