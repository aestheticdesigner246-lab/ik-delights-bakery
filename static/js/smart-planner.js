/* =========================================================
   ...
   ✅ Modal open -> first question INSTANTLY (with VOICE)
   ✅ MAXIMUM VOLUME (1.0) + LOUDEST VOICE SELECTION
   ✅ All 19 questions with conditional logic
   ✅ FIRST QUESTION SPEAKS RELIABLY (fixed)
========================================================= */
...
// ========== VOICE FUNCTION – MAX VOLUME + LOUDEST VOICE ==========
let preferredVoice = null;
let speechWarmedUp = false;

function selectLoudestVoice() {
    ...
    for (let name of priorityList) {
        let found = voices.find(v => v.name === name);
...
function warmupSpeech() {
    ...
    // Speak a very short, low-volume utterance to unlock the API
...
if (window.speechSynthesis) {
    ...
    window.speechSynthesis.onvoiceschanged = () => {
        preferredVoice = selectLoudestVoice();
...
function speakQuestion(text) {
    if (!text) return;
    // Cancel only if a different speech is ongoing (to avoid cutting off the same question)
    if (window.speechSynthesis && window.speechSynthesis.speaking) {
        window.speechSynthesis.cancel();
    }
    warmupSpeech();
    
    // Remove emojis for cleaner pronunciation
...
function renderQuestion() {
    ...
    // Speak the question – longer delay for first question (350ms) to ensure engine ready
    const delay = (SmartPlanner.currentQuestion === 0 && idx === 0) ? 350 : 80;
    setTimeout(() => {
...
// ========== MODAL OPEN – WITH WARMUP AND RELIABLE FIRST SPEECH ==========
window.openSmartEventPlanner = function() {
    ...
    renderQuestion();  // This will speak the first question after 350ms delay
    
    updateProgress();
    updateSummary();
    updateDynamicPackages();
    
    console.log("🗣️ Smart Event Planner opened – first question will be spoken loudly");
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("IK Delights Planner v3.0 - MAX VOLUME, first question speaks reliably");
});