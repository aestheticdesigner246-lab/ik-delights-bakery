/* ============================================
   IK DELIGHTS – FINAL PRODUCTION JS v25.1
   FIXED: Prioritize lastAddedProductType in checkout
   FIXED: Normalize product types (cookies → cookie, etc.)
   ============================================ */
...
async function showDealPopup() {
    ...
    try {
        ...
        if (waBtn && deal.title && deal.deal_price) {
            const businessNumber = "923214243501";
            const message = `Assalam o Alaikum, mujhe ${deal.title} deal order karni hai. Price: Rs ${deal.deal_price}`;
...
function proceedToCheckout() {
    ...
    // 1. Use lastAddedProductType (set during addToCart) – MOST RELIABLE
    if (lastAddedProductType && lastAddedProductType !== 'cake') {
        productType = lastAddedProductType;
    }
    // 2. Fallback to sessionStorage (in case of page reload)
    else if (sessionStorage.getItem('lastProductType')) {
        productType = sessionStorage.getItem('lastProductType');
    }
    // 3. Check cart item's product_type (if backend provides it)
    else if (cart[0] && cart[0].product_type) {
        productType = cart[0].product_type;
    }
    // 4. Last resort: guess from cart item name
    else if (cart[0] && cart[0].name) {
        let name = cart[0].name.toLowerCase();
        if (name.includes('cookie')) productType = 'cookie';
        else if (name.includes('brownie')) productType = 'brownie';
        else if (name.includes('bento')) productType = 'bento';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🔐 IK Delights v25.1 PRODUCTION READY');
